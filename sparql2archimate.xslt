...
<xsl:stylesheet version="1.0"
	...
	<xsl:output method="xml" 
		...
		<xsl:message>.</xsl:message>
			...
			<element>
		    		<name>
		    			<xsl:attribute name="identifier"><xsl:value-of select="./sparql:binding[@name='o']"/></xsl:attribute>
		    		...
		    		<xsl:if test="sparql:uri[text() = $s/sparql:uri/text()]">
	  				...
	  				<xsl:variable name="type" select=".//following-sibling::sparql:binding//following-sibling::sparql:binding/sparql:uri/text()" />
	  				
	  				<xsl:if test=".//following-sibling::sparql:binding/sparql:uri[text() = 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type']">
<xsl:attribute name="xsi:type">
		    				<xsl:value-of select="substring-after($type, '#')"/></xsl:attribute>
	  				</xsl:if>
		    		...
		    		<xsl:value-of select="substring-after($s, '_')"/>
		    		</name>
		    	</element>
